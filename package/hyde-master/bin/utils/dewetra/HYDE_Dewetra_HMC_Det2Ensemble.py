...
"""
Hyde Tools - Dewetra HMC Det2Ensemble Converter
...
python3 HYDE_Tools_Run_Det2Ensemble.py -settings_file configuration.json -time YYYYMMDDHHMM

Version:
20191017 (1.0.1) --> Refactor filename(s)
...
alg_name = 'HYDE TOOLS - DEWETRA HMC DET2ENSEMBLE CONVERTER'
alg_version = '1.0.1'
alg_release = '2019-10-17'
# ----------------------------------------------------------------------------------------------------------------------
...
def main():
    ...
    info_run, info_static, info_dynamic = file_configuration(config_file, time_arg)
    # ----------------------------------------------------------------------------------------------------------------------
    ...
    time_info = get_time(info_run['time']); print(time_info)
    ...
    file_out_def = None; print(time_arg)
    for section_data in section_info:
        ...
        for ensemble_id in ensemble_n_exp:
            ...
            template_out_values = {'datetime_outcome': time_info, 'sub_path_time': time_info,
                                   'ensemble': ensemble_name, 'section': section_name}; print(template_out_values)

            file_in_def = fill_tags2string(file_raw_in, template_info, template_in_values); print(file_in_def)
            file_out_def = fill_tags2string(file_raw_out, template_info, template_out_values); print(file_out_def)
...
def file_configuration(config_file, time_arg):
    with open(config_file) as config_handle:
        config_info = json.load(config_handle)
    info_run = config_info['info_run']; 
    if config_info['info_run']['time'] is None: 
    	config_info['info_run']['time'] = time_arg; 
    	info_run = config_info['info_run']
    info_static = config_info['info_static']
    info_dynamic = config_info['info_dynamic']; print(config_info)
