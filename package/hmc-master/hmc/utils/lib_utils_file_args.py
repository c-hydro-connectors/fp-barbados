...
def getDataSettings(sFileName, oSettings=FileSettings):

    [oFileData, bFileData] = importFileDict(sFileName, DataName='Data')

    if bFileData:
        oDictUpd = {}
        for sArgKey, oArgDict in iter(oSettings.items()):
            oDictUpd[sArgKey] = {}
            for sDictKey, oDictValue in iter(oArgDict.items()):
...
def checkFileSettings(sFileName, oSettings=FileSettings):
