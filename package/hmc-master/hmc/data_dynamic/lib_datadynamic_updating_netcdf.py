...
Name:          lib_datadynamic_updating_netcdf
...
# -------------------------------------------------------------------------------------
# Method to get gridded updating data
def getUpdatingGridded_Obs_NC(sDataTime, sRunMode='Deterministic', sDataFlag='OBS', sRefTime=None, iWaitTime=10,
                              oDataTags=ConfigTags,
                              oDataStatic=DataStatic_Default['DataAllocate']['Gridded'],
                              oDataGridded_IN=DataDynamic_Default['DataForcing']['Gridded'],
                              oDataGridded_OUT=DataDynamic_Default['DataAllocate']['Gridded'],
                              sPathTemp='/temp'):
        ...
        if oDataOp_OUT:
            ...
            if bFileFound is True:
                ...
                if oFileDriver.oFileLibrary.checkVarName(oFileHandle, sVarNameDyn):
                    ...
                    Exc.getExc(
                        ' =====> WARNING: variable ' + sVarKey + ' does not exist in input file!', 2, 1)
                    # -------------------------------------------------------------------------------------
                ...
                Exc.getExc(
                    ' =====> WARNING: file(s) ' + str(a1sFileNotFound) + ' are not found!', 2, 1)
                # -------------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------------
# Method to save gridded updating data
def saveUpdatingGridded_NC(sDataTime, sFileName,
                                  sRunMode='Deterministic', sRunType='OBS',
                                  oDataSettings=FileSettings,
                                  oDataTags=ConfigTags,
                                  oDataGridded=DataDynamic_Default['DataAllocate']['Gridded']):
