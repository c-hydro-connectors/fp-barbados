...
#-----------------------------------------------------------------------------------------
# Script information
script_name='HMC RUNNER - RF WRF - REALTIME'
...
setting_file_hmc='/share/c-hydro/hmc-master/config/algorithms/hmc_model_run-manager_algorithm_rf_wrf_realtime.config'

script_file_dewetra='/share/c-hydro/hyde-master/bin/utils/dewetra/HYDE_Dewetra_HMC_Det2Ensemble.py'
setting_file_dewetra='/share/c-hydro/hyde-master/bin/utils/dewetra/hyde_dewetra_hmc_rf_wrf_det2ensemble_realtime.json'

env_flag=true

folder_lock_raw='/share/c-hydro/lock/'
file_lock_start_raw='hmc_runner_rf_wrf_lock_%YYYY%MM%DD%HH00_realtime_START.txt'
file_lock_end_raw='hmc_runner_rf_wrf_lock_%YYYY%MM%DD%HH00_realtime_END.txt'

folder_name_list=('/share/c-hydro/lock/' '/share/c-hydro/lock/' '/share/c-hydro/lock/' '/share/c-hydro/lock/')
file_name_list=('nwp_dynamicdata_wrf_lock_%YYYY%MM%DD%HH00_realtime_START.txt' 'nwp_dynamicdata_wrf_lock_%YYYY%MM%DD%HH00_realtime_START.txt'
				 'rf_model_wrf_lock_%YYYY%MM%DD%HH00_realtime_START.txt' 'rf_model_wrf_lock_%YYYY%MM%DD%HH00_realtime_END.txt')

# Get information (-u to get gmt time)
time_system=$(date +"%Y-%m-%d %H:00")
# time_system=$(date -u +"%Y-%m-%d %H:00")
#time_system='2021-08-04 09:32' # DEBUG 
#-----------------------------------------------------------------------------------------
...
if $file_check; then
    ...
    if [ -f $path_file_lock_def_start ] && [ -f $path_file_lock_def_end ]; then
		...
		python3 $script_file_dewetra -settings_file $setting_file_dewetra -time $time_run
