...
from fp.utils.lib_utils_apps_geo import createMeshGrid
from fp.utils.lib_utils_op_string import defineString
from fp.utils.lib_utils_op_dict import mergeDict
from fp.utils.lib_utils_op_list import mergeList
from fp.utils.lib_utils_apps_data import updateDictStructure, squeezeDataArray
...
from fp.driver.dataset.nwp.wrf.cpl_data_variables_wrf import DataVariables
            ...
            if oVarWS:
                ...
                if oVarSel[sVarKey]:
                    ...
                    a3dVarValue_CMP, oVarTimes_CMP, oVarAttrs_CMP = oDrv_Data_Cmp.computeVarData(
                        type=oVarType[1], index_start=1, index_end=49)
                    ...
                    if all(oVarCheck is not None for oVarCheck in [a3dVarValue_CMP, oVarTimes_CMP, oVarAttrs_CMP]):
                        ...
                        if ('values' in oVarSel[sVarKey]) and ('attributes' in oVarSel[sVarKey]) and ('times' in oVarSel[sVarKey]):
                            ...
                            a3dVarValue_FILTER = zeros([oDataGeo.a2dGeoX.shape[0], oDataGeo.a2dGeoY.shape[1], oVarTime.__len__()])
                            ...
                            for sVarName in oVarName:
                                ...
                                if sVarName not in oVarWS:
                                    ...
                                    if oFileDriver.oFileLibrary.checkVarName(oFileHandle, sVarName):
                                        ...
                                        if oVarTimes[0] != sVarTime:
                                            sTimeFound = oVarTimes[0]
                                            sTimeExp = sVarTime
                                            oLogStream.warning(' ===> TimeExpected: ' + sTimeExp +
                                                               ' TimeFound: ' + sTimeFound)
                                            oLogStream.warning(' ===> Set TimeExpected as Reference')

                                            oVarTimes[0] = sTimeExp
