...
__library__ = 'flood_proofs'

General command line:
python3 fp_downloader_drops_ws.py -settingfile configuration_product.json
...
from fp.utils.lib_utils_apps_time import convertTimeFrequency

from fp.dataset.db.drops.lib_db_drops_apps_generic import initDropsDB
from fp.dataset.db.drops.lib_db_drops_apps_sensor import getSensorType, getSensorRegistry, getSensorData
from fp.dataset.db.drops.lib_db_drops_utils_sensor_ws import parseSensorData, parseSensorRegistry, createSensorDataFrame
from fp.dataset.db.drops.lib_db_drops_utils_io import writeSensorDataFrame
# -------------------------------------------------------------------------------------
        ...
        for sVarKey, oVarAttributes in oData_VAR.items():
            ...
            if bVarDownload is True:
                ...
                if (not exists(join(sFolder_WS, sFileName_WS))) or (bFileName_WS is True):
                    ...
                    oTime_RANGE = date_range(end=sTime_STEP, 
                                            periods=2, 
                                            freq=oDataSettings['time_info']['time_frequency'])
